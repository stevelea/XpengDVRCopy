#!/bin/bash
# copyusb.sh - copy car-camera (DVR) footage from a plugged-in USB drive to a NAS share.
#
# Installed by install.sh to /usr/local/bin/copyusb.sh
# Part of XpengDVRCopy: https://github.com/stevelea/XpengDVRCopy
...
# ----------------------------------------------------------------------------
NAS_IP=""                     # informational only; the real target is MOUNTPOINT
...
MQTT_ENABLED="1"
MQTT_HOST=""                  # e.g. 198.51.100.5 (blank = reporting off)
MQTT_PORT="1883"
MQTT_USER=""
MQTT_PASS=""                  # set in /etc/xpg-camera-copy.conf or install.sh
...
MQTT_TOPIC_CMD="${MQTT_PREFIX}/cmd"              # HA switch writes here
...
publish_last_copy() {
    ...
    timeout "$MQTT_TIMEOUT" mosquitto_pub -h "$MQTT_HOST" -p "$MQTT_PORT" \
        -u "$MQTT_USER" -P "$MQTT_PASS" -q 1 -r \
        -t "$MQTT_TOPIC_LAST" -m "$(now)" 2>/dev/null
}
